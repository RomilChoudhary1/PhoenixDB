-module(phoenix_folsom_graphite_server).
...
-define(FOLSOM_ARCHIVE_TABLE, folsom_archive_ets_table).
...
-record(state, {
          ...
          vm_metrics = [] :: list(),
          sup_pid = undefined :: undefined | pid(),
...
start_link(Index, SupPid) ->
    ...
    gen_server:start_link({local, Name}, ?MODULE, [FolsomDdbConfig, BucketIndexConfig, SupPid], []).
...
%% @end
%%--------------------------------------------------------------------
init([FolsomDdbConfig, BucketIndexConfig, SupPid]) ->
    ...
    case proplists:get_value(enabled, FolsomDdbConfig, false) of
        true ->
            ...
            Bucket = phoenix_util:convert_to_binary(BucketS ++ BucketSuffix),
            ...
            create_ets_table(?FOLSOM_ARCHIVE_TABLE, SupPid),
            {ok, State#state{sock = undefined, ref = Ref, sup_pid = SupPid}};
        false -> {ok, #state{}}
    end.
...
handle_info({timeout, _R, tick},
    ...
    {_UpdatedArchivedCounters, Sock1} = aggregate_and_send_metrics(
        NewSock, Dimensions, VMSpec, ?FOLSOM_ARCHIVE_TABLE, TablePrefixes),
...
%%    lager:debug("Elapsed ~p msec to collect and send metrics", [MicroSec / 1000]),
    Ref = erlang:start_timer(Interval, self(), tick),
    {noreply, State#state{ref = Ref, sock = Sock1}};
...
create_ets_table(TableName, SupPid) ->
...
do_metrics(Dimensions,
    ...
    case is_metric_in_table_prefix(N, TablePrefixes) of
        {true, _TablePrefix} ->
            ...
            DeltaCount = try
                             OldCount = ets:lookup_element(ArchivedCounters, N, 2),
                             Count - OldCount
                         catch
                             _:_ -> Count
                         end,
            ...
            case Socket1 of
                undefined ->
                    {ArchivedCounters, Socket};
                _ ->
                    ets:insert(ArchivedCounters, {N, Count}),
                    do_metrics(Dimensions, Spec, ArchivedCounters, TablePrefixes, Socket1)
            end;
        {false, _} ->
...
"~s ~p ~p~n", [Metric1, Value, phoenix_util:get_utc_seconds()])),
        [MetricValue | AccIn]
                  end, [], MVs),
...
"~s ~p ~p~n", [Metric1, RoundedValue, phoenix_util:get_utc_seconds()])),
...
folsom_metric_to_parts(<<"sm.redis.", Rest/binary>>) ->
    Parts = binary:split(Rest, <<".">>, [global, trim]),
    case Parts of
        [OperationType, PoolName, Result, OrgUnit] ->
            {[?METRIC_PREFIX, <<"redis">>],
             [<<"redisop=", OperationType/binary>>, <<"pool=", PoolName/binary>>, <<"result=", Result/binary>>,
              <<"orgunit=", OrgUnit/binary>>]};
        [OperationType, PoolName, Result, <<"ms">>, OrgUnit] ->
                {[?METRIC_PREFIX, <<"redis-hist">>],
                    [<<"redisop=", OperationType/binary>>, <<"pool=", PoolName/binary>>, <<"result=", Result/binary>>,
                        <<"orgunit=", OrgUnit/binary>>]};
        _ ->
            undefined
    end;
folsom_metric_to_parts(<<"bm.event.", Rest/binary>>) ->
    Parts = binary:split(Rest, <<".">>, [global, trim]),
    case Parts of
        [Classifier, Service, Result] ->
            {[?METRIC_PREFIX, <<"bm">>, <<"event">>],
                [<<"name=", Classifier/binary>>,
                    <<"service=", Service/binary>>,
                    <<"result=", Result/binary>>]};
        [Classifier, Service, <<"ms">>, Result] ->
            {[?METRIC_PREFIX, <<"bm">>, <<"event">>],
                [<<"name=", Classifier/binary>>,
                    <<"service=", Service/binary>>,
                    <<"result=", Result/binary>>]};
