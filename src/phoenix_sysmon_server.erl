...
handle_info({timeout, _R, tick},
    #state{ref = _Ref, interval = FlushInterval,
...
do_http_metrics(Prefix) ->
    try
        OmsListenerActiveConnections = ranch_server:count_connections(http),
        log_utils:publish_gauge(
            iolist_to_binary(
                [Prefix, <<"http.phoenix_listener.active_connections">>]),
            OmsListenerActiveConnections),
        lager:debug("[~p] ~p Active Connections = ~p",
            [self(), ?MODULE, OmsListenerActiveConnections])
    catch
        C:E ->
...
publish_cpu_metric(_, _Prefix) ->
    %% for microsoft windows cpu statistics are not supported
    ok.
-else.
publish_cpu_metric(avg1, Prefix) ->
